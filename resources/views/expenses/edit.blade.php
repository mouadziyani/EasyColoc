            ...
            <div class="lg:col-span-8">
                <form action="{{ route('colocations.expenses.store', $colocation) }}" method="POST" class="space-y-12">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-12">
                        ...
                        <div class="relative group">
                            <label class="text-[10px] uppercase tracking-widest font-bold text-gray-400 group-focus-within:text-black">Payé par</label>
                            <select name="paid_by" class="w-full border-0 border-b border-gray-200 py-4 px-0 focus:ring-0 focus:border-black transition-all text-lg bg-transparent" required>
                                <option value="" class="text-gray-400">Sélectionner un membre...</option>
                            ...
                            @error('paid_by') <span class="text-xs text-red-600 mt-2 block font-mono italic">{{ $message }}</span> @enderror
                        </div>
                    </div>
